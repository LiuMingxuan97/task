...
q0, q1,  q2, q3 = 0.48252688, -0.03286932, 0.87449189, 0.036761
q_list = [q0, q1,  q2, q3]
...
pix_cam_id = (3960,3960)
principal_point_x = 3960
...
1 建立 pos_cam_mm 
2 建立 R_img2cam
3 建立 R_cam2body 目前为单位矩阵 描述相机对于卫星本体的姿态
4 建立 R_body2orb 传入欧拉角构建旋转矩阵
5 对地固系下位置、速度 使用 ecr2eci 转换为地惯系下速度、位置
6 建立 R_orb2eci 使用 5 中地惯系下位置速度构建
7 如果传入为 四元数 建立 R_body2eci_q 代替 R_body2orb * 
7 建立 R_eci2ecr 需要传入当前时间
8 
'''
...
def major(model):
    ...
    ecef_pos =  np.array(eci_pos).dot(R_eci2ecr)
    print('ecef_pos', ecef_pos)
    ...
    ground_point = cal_pos(major_radius, minor_radius, v_i, obs_pos)
    print('ground_point:\n', ground_point)
